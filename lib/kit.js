...
function Kit(dom) {
  ...
  def(this, 'parent', function () {
    return newKit((this.dom[0] || {}).parent);
  });
...
Kit.prototype.find = function (selector) {
  return newKit(cssSelect(selector, this.dom, { xmlMode: true }));
};
...
Kit.prototype.slice = function () {
  return newKit(this.dom.slice.apply(this.dom, arguments));
};
...
Kit.prototype.sort = function () {
  var nodes = this.dom.sort.apply(this.dom.map(newKit), arguments);
  return newKit(_(nodes).pluck('dom').flatten().value());
};
...
Kit.prototype.filter = function () {
  var nodes = this.dom.filter.apply(this.dom.map(newKit), arguments);
  return newKit(_(nodes).pluck('dom').flatten().value());
};
...
Kit.prototype.forEach = function () {
  this.dom.forEach.apply(this.dom.map(newKit), arguments);
  return this;
};
...
Kit.prototype.map = function () {
  return this.dom.map.apply(this.dom.map(newKit), arguments);
};
...
Kit.prototype.reduce = function (fn) {
  return this.dom.reduce.apply(this.dom, [function (acc, node) {
    return fn.apply(this, [acc, newKit(node)].concat([].slice.call(arguments, 2)));
...
Kit.prototype.remove = function (kit) {
  ...
  kit.dom.forEach(function (node) {
    ...
    if (index > -1) this.dom.splice(index, 1);
  }, this);
  ...
  kit.dom.forEach(function (node) {
    ...
    if (node.prev) {
      ...
      (node.children || []).forEach(function (child) {
        domUtils.prepend(node.next, child);
      });
    } else if (node.parent) {
      (node.children || []).forEach(function (child) {
