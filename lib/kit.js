...
function Kit(dom) {
  ...
  def(this, 'attr', function () {
    var dom = (this.dom[0] || {});
    return dom.attribs || (dom.attribs = {});
  }, function (value) {
      /*jshint boss:true */
...
Kit.prototype.append = function (kit) {
  var dom = this.dom.reduce(function (kit, dom, node) {
    var siblings = kit.clone().dom.reverse().reduce(function (dom, sib) {
      domUtils.append(node, sib);
      return [sib].concat(dom);
    }, []);

    return dom.concat([node]).concat(siblings);
  }.bind(this, factory(kit)), []);

  this.dom.splice.apply(this.dom, [0, this.dom.length].concat(dom));
  return this;
};
...
Kit.prototype.prepend = function (kit) {
  var dom = this.dom.reduce(function (kit, dom, node) {
    var siblings = kit.clone().dom.reduce(function (dom, sib) {
      domUtils.prepend(node, sib);
      return dom.concat([sib]);
    }, []);

    return dom.concat(siblings).concat([node]);
  }.bind(this, factory(kit)), []);
...
Kit.prototype.removeEmpty = function () {
  var kit;
  while ((kit = this.find(':empty')).length)
    this.replace(kit);
  return this;
};
...
Kit.prototype.replace = function (src, dst) {
  ((src instanceof Kit) ? src : this.find(src)).dom.reverse().forEach(function (dst, src) {
    replaceAndSplice(this, factory(src), dst);
